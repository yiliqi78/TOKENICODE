...
def main() -> int:
    ...
    # Validate next_action completeness
    STANDARD_ACTIONS = ["research", "implement", "check", "review", "finish", "create-pr"]
    next_action = task_data.get("next_action", [])
    existing_actions = [item.get("action") for item in next_action if isinstance(item, dict)]
    current_phase_val = task_data.get("current_phase", 0) or 0
    if current_phase_val == 0:
        # Fresh task — auto-fix all missing standard phases
        missing_actions = [a for a in STANDARD_ACTIONS if a not in existing_actions]
        if missing_actions:
            log_warn(f"next_action missing phases: {', '.join(missing_actions)}")
            log_info("Auto-fixing: adding missing phases to next_action")
            for action in missing_actions:
                next_action.append({"action": action})
            for i, item in enumerate(next_action, 1):
                item["phase"] = i
            task_data["next_action"] = next_action
            write_json(task_json_path, task_data)
            log_info(f"Fixed next_action: {[item['action'] for item in next_action]}")
    else:
        # Mid-pipeline task — only warn, never rewrite (would corrupt in-progress pipelines)
        missing_actions = [a for a in STANDARD_ACTIONS if a not in existing_actions]
        if missing_actions:
            log_warn(f"next_action missing phases (mid-pipeline, NOT auto-fixing): {', '.join(missing_actions)}")

    # Validate jsonl context files exist
    for jsonl_name in ["implement.jsonl", "check.jsonl"]:
        jsonl_path = task_dir_abs / jsonl_name
        if not jsonl_path.is_file():
            log_warn(f"{jsonl_name} not found — hook context injection will use fallback")
